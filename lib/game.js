...
class Game {
  ...
  checkCollisions() {
    this.allBubbles().forEach((bubble) => {
      if (bubble!== -1) {
        if ((this.checkCollision(bubble.bubble, this.currentBubble)) || this.currentBubble.bubble.y <= 0) {
          this.turns -= 1;
          this.snapBubble();
    ...
    this.allBubbles().forEach( bubble => {
      if (bubble !== -1) {
        if (!bubble.processed) {
          ...
          if (cluster.every( bubble => bubble.bubble.y !== 0)){
  ...
  checkGameOver() {
    ...
    if (!this.gameOver) this.checkLost();
  }
  ...
  checkLost() {
    this.allBubbles().forEach(bubble => {
      if (bubble !== -1) {
        if ((bubble.bubble.y > 305 && bubble.vy === 0) || this.turns === 0) {
