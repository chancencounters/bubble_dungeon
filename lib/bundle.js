	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    ...
	    value: function checkCollisions() {
	      ...
	      this.allBubbles().forEach(function (bubble) {
	        if (bubble !== -1) {
	          if (_this3.checkCollision(bubble.bubble, _this3.currentBubble) || _this3.currentBubble.bubble.y <= 0) {
	            _this3.turns -= 1;
	            _this3.snapBubble();
	            _this3.burstMatching(_this3.currentBubble);
	    ...
	    value: function snapBubble() {
	      ...
	      var pos = this.currentBubble.getBoardPosition();
	      ...
	      while (this.bubbles[pos.row][pos.col] instanceof Bubble) {
	        this.currentBubble.bubble.x += this.currentBubble.inverseVx;
	        this.currentBubble.bubble.y += this.currentBubble.inverseVy;
	    ...
	    value: function checkLost() {
	      ...
	      this.allBubbles().forEach(function (bubble) {
	        if (bubble !== -1) {
	          if (bubble.bubble.y > 305 && bubble.vy === 0 || _this7.turns === 0) {
