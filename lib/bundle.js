	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    ...
	    value: function remainingColors() {
	      var colors = [Constants.blue, Constants.red, Constants.yellow, Constants.green];
	      var remainingColors = [];
	    ...
	    value: function getBubbleCoords(x, y) {
	      ...
	      var bubbley = y * Constants.rowHeight + Constants.radius;
	    ...
	    value: function checkCollisions() {
	      ...
	      this.allBubbles().forEach(function (bubble) {
	        if (bubble !== -1) {
	          if (_this2.checkCollision(bubble.bubble, _this2.gun.currentBubble) || _this2.gun.currentBubble.y <= 16) {
	            _this2.turns -= 1;
	            _this2.gun.currentBubble.snapBubble();
	            _this2.addBubbleToBoard();
	            _this2.burstMatching(_this2.gun.currentBubble);
	            _this2.gun.updateCurrentBubble(_this2.remainingColors());
	    ...
	    value: function dropFloatingBubbles() {
	      ...
	      this.allBubbles().forEach(function (bubble) {
	        if (bubble !== -1) {
	          if (!bubble.processed) {
	            ...
	            var cluster = _this4.findFloating(bubble);
	      ...
	      floating.forEach(function (bubble) {
	        ...
	        window.setTimeout(function () {
	          _this4.bubbles[pos.row][pos.col] = -1;
	          _this4.stage.removeChild(bubble.bubble);
	        }, 1000);
	      });
	    }
	  }, {
	    ...
	    value: function checkLost() {
	      ...
	      this.allBubbles().forEach(function (bubble) {
	        if (bubble.y > 305 && bubble.vy === 0 || _this6.turns === 0) {
	          _this6.renderLoseMsg();
	          if (_this6.gameOver === false) {
	            window.setTimeout(function () {
	              _this6.reset();
	            }, 2000);
	            _this6.gameOver = true;
	          }
	        }
	      });
	    }
	    ...
	    key: "renderLoseMsg",
	    value: function renderLoseMsg() {
	      var text = new createjs.Text("You Lost! Try again!", "40px Arial", "#ffffff");
	      text.x = 35;
	      text.y = 200;
	      text.textBaseline = "alphabetic";
	      this.stage.addChild(text);
	    ...
	    key: "renderWinMsg",
	    value: function renderWinMsg() {
	      var text = new createjs.Text("You Win!", "80px Arial", "#ffffff");
	      text.x = 55;
	      text.y = 200;
	      text.textBaseline = "alphabetic";
	      this.stage.addChild(text);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Gun = function () {
	  function Gun(game, stage, colors) {
	  ...
	  _createClass(Gun, [{
	    ...
	    value: function setCurrentBubble(colors) {
	      var pos = { x: 190, y: 355 };
	      var currentBubble = new Bubble(pos, colors);
